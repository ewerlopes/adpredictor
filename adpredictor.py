...
class AdPredictor(object):
    ...
    def _get_weight(self, feature):
        return self._weights.get(util.serialize_feature(feature), util.prior_weight())

    def _set_weight(self, feature, weight):
        logger.debug(
            "Setting feature: %s frow weight: %s to weight: %s",
            util.pp(feature),
    ...
    def _apply_dynamics(self, weight):
        ...
        logger.debug("Adjusting weight %s to %s", util.pp(weight), util.pp(adjusted))
        return adjusted

    def _importance(self, feature):
        prior_prediction = self.predict(
            [util.bias_feature()] + [pb.Feature()] * (self._config.num_features - 1)
        )
        with_weight_prediction = self.predict(
            [util.bias_feature()]
            + [pb.Feature()] * (self._config.num_features - 2)
            + [feature]
        )
