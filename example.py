...
class Sampler(object):
    ...
    def _construct_biased_weights(simulation):
        ...
        for feature, value in itertools.product(
            ...
            if np.random.rand() < simulation.biased_feature_proportion:
                direction = (
                    np.random.rand() < simulation.predictor_config.prior_probability
                )
    ...
    def _default_label(self):
        return np.random.rand() < self._simulation.predictor_config.prior_probability
...
class SimulationRunner(object):
    ...
    def _plot_weights(self):
        for color, (feature, weights) in itertools.izip(
            itertools.cycle(self.COLORS), self._current_weights_by_feature()
        ):
    ...
    def run(self):
        samples = itertools.islice(self._sampler, self._simulation.num_examples)
